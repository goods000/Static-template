...
               //  if(data&&LastLength!=data.length){
				if(data&&data.length){		//自己改的
                        onDataCallback(data, { noData: false });
                 }else{
                         onDataCallback([], { noData: true });
                }
				
				//LastLength = data.length;			//自己改的
...
    return {
     'name': this.self.symbol.toLocaleUpperCase(),		//自己改动
...
      'description': this.self.symbol.toLocaleUpperCase()+'USDT',
          //设置精度  100表示保留两位小数   1000三位   10000四位
      'pricescale': 100,
      'ticker': this.self.symbol.toLocaleUpperCase()+'USDT',
//      'supported_resolutions': ['1', '5', '15', '30', '60', '240','1D', '5D', '1W', '1M']
		'supported_resolutions': ['1', '5', '15', '30', '60', '1D', ]
    };
  };
