...
// 限制字符长度
function maxLen(val,max,string){
	if(val.length>max){
		layer.msg(string+"不能超过"+max+"个字")
		return false;
	}
	return true;
}
...
function telCheck(value){
	console.log(value);
	// var regex =  /^((1[0-9])+\d{9})$/;
	var regex =  /^(13[0-9]|14[579]|15[0-3,5-9]|16[6]|17[0135678]|18[0-9]|19[89])\d{8}$/; //手机号 
	if(value==""){
		layer.msg("请输入手机号");
		return false;
...
// 6位密码数字不能重复
function Check_payPwd(value,string){
	console.log(value.length)
		if(value.length != 6){
			// mui.alert("",payError1,btn)
			layer.msg("请输入6位数字的"+string);
			return false;
		}	
		// var reg1=/([0-9])\1{2}/
		// var reg2=/([0-9])\1{3}/
		...
		// 	layer.msg(string+"是6位密码数字不能重复");
		// 	return false;
		// }
		if(Checknumber(value,string)){
			return true;
		}else{
			return false;
		}
...
function checkUserName(value,string) {
	var regg = /^[a-zA-Z0-9_]{1,}$/;
	if(!regg.test(value)) {
		layer.msg(string+"只能输入字母和数字下划线");
		return false;
	}
	return true;
}
